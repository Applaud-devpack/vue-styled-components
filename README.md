...
```
yarn add @applaud/vue-styled-components
```
...
```JS
import styled from '@applaud/vue-styled-components';
...
```JSX
import styled from '@appalud/vue-styled-components';
...
`@applaud/vue-styled-components` has full theming support by exporting a `<ThemeProvider>` wrapper component. This component provides a theme to all `Vue` components underneath itself via the context API. In the render tree all `@applaud/vue-styled-components` will have access to the provided theme, even when they are multiple levels deep.

Remember to register `ThemeProvider` locally.

```JSX
  import {ThemeProvider} from '@applaud/vue-styled-components'
...
import { injectGlobal } from '@applaud/vue-styled-components';
